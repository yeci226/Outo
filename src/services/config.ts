...
class ConfigService {
	...
	loadConfig(): AppConfig {
		try {
			...
			const config: AppConfig = {
				...
				outo: {
					maxLogsPerGuild: parseInt(
						process.env.OUTO_MAX_LOGS_PER_GUILD || "10"
					),
					logRetentionDays: parseInt(
						process.env.OUTO_LOG_RETENTION_DAYS || "30"
					),
					emojiCacheEnabled: process.env.OUTO_EMOJI_CACHE === "true",
					autoCleanupInterval: parseInt(
						process.env.OUTO_AUTO_CLEANUP_INTERVAL || "3600000"
					) // 1小時
				},
			...
			this.logger.success("Outo設定載入完成");
			return config;
		} catch (error) {
	...
	// 取得Outo特定設定
	getOutoConfig(): OutoConfig {
		return this.get("outo", {});
	}
	...
	validateConfig(): boolean {
		...
		this.logger.success("Outo設定驗證通過");
		return true;
	}
